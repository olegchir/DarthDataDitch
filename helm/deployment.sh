...
deploy_darthdata_ditch() {
  ...
  echo "DEBUG"
  echo "upgrade --install helmdataditch ./darthdataditch-${CHART_VERSION}.tgz  -f darthdataditch/values.yaml -f  darthdataditch/values-${DEPLOYMENT_REGION}.yaml"
  helm upgrade --install helmdataditch ./darthdataditch-${CHART_VERSION}.tgz  -f darthdataditch/values.yaml -f  darthdataditch/values-${DEPLOYMENT_REGION}.yaml
}
